...
ALLOWED_SOURCES = {"manual-csv", "auto-mutual-fund"}
    ...
    with INPUT_PATH.open(encoding="utf-8", newline="") as handle:
        ...
        for line_number, row in enumerate(reader, start=2):
            ...
            if source not in ALLOWED_SOURCES:
                raise ValueError(f"line {line_number}: source must be manual-csv / auto-mutual-fund")
...
def validate_payload(payload: dict, rows: list[dict]) -> dict[str, int]:
    ...
    for code, item in prices.items():
        ...
        if item.get("source") not in ALLOWED_SOURCES:
            raise ValueError(f"{code}: source must be manual-csv / auto-mutual-fund")
